...
export default function ProductEditModalNew({ product, onSave, onCancel, isSaving }: ProductEditorProps) {
  const [formData, setFormData] = useState<Partial<Product>>({
    ...
    instructionManualUrl: product.instructionManualUrl || '',
    // Customs & Compliance Fields
    hsCode: product.hsCode || '',
    countryOfOrigin: product.countryOfOrigin || 'CA',
    customsDescription: product.customsDescription || '',
    defaultCustomsValueCad: product.defaultCustomsValueCad || 0,
    massGrams: product.massGrams || 0,
  });

  const [availableImages, setAvailableImages] = useState<ImageOption[]>([]);
  const [selectedImageUrls, setSelectedImageUrls] = useState<string[]>(['', '', '', '']);
  const [isLoadingImages, setIsLoadingImages] = useState(true);
  const [variants, setVariants] = useState<Array<{ id?: string; style: string; price: number; quantity: number; sku?: string }>>(product?.variants || []);
  ...
  // Initialize selected images from existing product
  useEffect(() => {
              ...
              <div className="mt-4 p-4 bg-blue-500/10 border border-blue-500/20 rounded-lg">
                <div className="flex items-start">
                  ...
                  <div className="text-sm text-blue-300">
                    <p className="font-medium mb-1">How to update images:</p>
                    <ol className="list-decimal list-inside space-y-1 text-blue-200">
                      <li>Upload new images using the <a href="/admin/file-manager" target="_blank" className="underline">File Manager</a></li>
                      <li>Refresh this page to see new images in the dropdown</li>
                      <li>Select different images from the dropdowns</li>
